...
import * as mod from "./modular.mjs";
import * as ut from "./utils.mjs";
import * as utils_1 from "./utils.mjs";
import * as curve_1 from "./curve.mjs";
...
export function weierstrassPoints(opts) {
    ...
    function normalizePrivateKey(key) {
        ...
        let num;
        try {
            num = typeof key === 'bigint' ? key : ut.bytesToNumberBE(utils_1.ensureBytes(key, nByteLength));
        }
        catch (error) {
    ...
    class Point {
         ...
         */
        static fromHex(hex) {
            const P = Point.fromAffine(CURVE.fromBytes(utils_1.ensureBytes(hex)));
            P.assertValidity();
            return P;
        }
    ...
    const wnaf = curve_1.wNAF(Point, CURVE.endo ? Math.ceil(_bits / 2) : _bits);
    return {
...
function validateOpts(curve) {
    ...
    return Object.freeze(Object.assign({ lowS: true }, opts));
}
...
export function weierstrass(curveDef) {
    ...
    const { ProjectivePoint: Point, normalizePrivateKey, weierstrassEquation, isWithinCurveOrder, } = weierstrassPoints(Object.assign(Object.assign({}, CURVE), { toBytes(c, point, isCompressed) {
    ...
    class Signature {
        ...
        static fromCompact(hex) {
            ...
            hex = utils_1.ensureBytes(hex, gl * 2);
        ...
        static fromDER(hex) {
            ...
            const { r, s } = DER.toSig(utils_1.ensureBytes(hex));
        ...
        recoverPublicKey(msgHash) {
            ...
            const h = bits2int_modN(utils_1.ensureBytes(msgHash)); // Truncate hash
    ...
    function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
        ...
        if (prehash)
            msgHash = hash(utils_1.ensureBytes(msgHash));
        if (lowS == null)
            lowS = true; // RFC6979 3.2: we skip step A, because we already provide hash
        // We can't later call bits2octets, since nested bits2int is broken for curves
        // with nBitLength % 8 !== 0. Because of that, we unwrap it here as int2octets call.
        // const bits2octets = (bits) => int2octets(bits2int_modN(bits))
        const h1int = bits2int_modN(utils_1.ensureBytes(msgHash));
        ...
        if (ent != null) {
            ...
            seedArgs.push(utils_1.ensureBytes(ent === true ? randomBytes(Fp.BYTES) : ent, Fp.BYTES));
        }
    ...
    function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
        ...
        try {
            ...
            msgHash = utils_1.ensureBytes(msgHash);
        ...
        const R = (_a = Point.BASE.multiplyAndAddUnsafe(P, u1, u2)) === null || _a === void 0 ? void 0 : _a.toAffine(); // R = u1⋅G + u2⋅P
        if (!R)
            return false;
        const v = modN(R.x);
        return v === r;
...
export function SWUFpSqrtRatio(Fp, Z) {
    ...
    const c2 = (q - 1n) / Math.pow(2n, c1); // 2. c2 = (q - 1) / (2^c1)        # Integer arithmetic
    const c3 = (c2 - 1n) / 2n; // 3. c3 = (c2 - 1) / 2            # Integer arithmetic
    const c4 = Math.pow(2n, c1) - 1n; // 4. c4 = 2^c1 - 1                # Integer arithmetic
    const c5 = Math.pow(2n, (c1 - 1n)); // 5. c5 = 2^(c1 - 1)              # Integer arithmetic
    ...
    let sqrtRatio = (u, v) => {
        ...
        for (let i = c1; i > 1; i--) {
            let tv5 = Math.pow(2n, (i - 2n)); // 18.    tv5 = i - 2;    19.    tv5 = 2^tv5
